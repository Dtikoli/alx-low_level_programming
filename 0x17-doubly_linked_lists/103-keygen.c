...
int main(int ac, char *av[])
{
	...
	char *str = (char *) nums, *uname = av[1], *ptr, pword[7];
	...
	for (ptr = uname; *ptr; ptr++)
		len++;
	pword[0] = str[(len ^ 0x3b) & 0x3f];
	for (ptr = uname, temp = 0; *ptr; ptr++)
		temp += *ptr;
	pword[1] = str[(temp ^ 0x4f) & 0x3f];
	for (ptr = uname, temp = 1; *ptr; ptr++)
		temp *= *ptr;
	pword[2] = str[(temp ^ 0x55) & 0x3f];
	for (ptr = uname, temp = *ptr; *ptr; ptr++)
		if (temp < *ptr)
			temp = *ptr;
	srand(temp ^ 0xe);
	pword[3] = str[rand() & 0x3f];
	for (ptr = uname, temp = 0; *ptr; ptr++)
		temp += *ptr * *ptr;
	pword[4] = str[(temp ^ 0xef) & 0x3f];
	for (temp = *uname, ct = 0; ct < temp; ct++)
		randh = rand();
	pword[5] = str[(randh ^ 0xe5) & 0x3f];
	printf("%s", pword);
	return (0);
}
